...
errno_t image_basic_resize_cli()
{
    ...
    {
        basic_resizeim(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.string, data.cmdargtoken[3].val.numl,
...
errno_t imresize_addCLIcmd()
{
    ...
    RegisterCLIcommand(
        "resizeim", __FILE__, image_basic_resize_cli, "resize 2D image",
        "<image in> <output image> <new x size> <new y size>", "resizeim im1 im2 230 200",
        "long basic_resizeim(const char *imname_in, const char *imname_out, long xsizeout, long ysizeout)");
            ...
            for (jj = 0; jj < naxesout[1]; jj++)
            {
                ...
                if ((ii1 > -1) && (ii1 + 1 < naxes[0]) && (jj1 > -1) && (jj1 + 1 < naxes[1]))
                {
                    ...
                    data.image[IDout].array.F[jj * naxesout[0] + ii] =
                        (float)(v00f * (1.0 - uf) * (1.0 - tf) + v10f * uf * (1.0 - tf) + v01f * (1.0 - uf) * tf +
                                v11f * uf * tf);
                }
            }
    }
        ...
        for (ii = 0; ii < naxesout[0] - 1; ii++)
            for (jj = 0; jj < naxesout[1] - 1; jj++)
            {
                ...
                data.image[IDout].array.D[jj * naxesout[0] + ii] =
                    (double)(v00d * (1.0 - ud) * (1.0 - td) + v10d * ud * (1.0 - td) + v01d * (1.0 - ud) * td +
                             v11d * ud * td);
            }
    }
    else
