...
static errno_t image_basic_contract_cli()
{
    ...
    {
        basic_contract(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.string, data.cmdargtoken[3].val.numl,
...
static errno_t image_basic_contract3D_cli()
{
    ...
    {
        basic_contract3D(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.string, data.cmdargtoken[3].val.numl,
                ...
                for (uint32_t kk = 0; kk < naxes_out[2]; kk++)
                    for (int i = 0; i < n1; i++)
                        for (int j = 0; j < n2; j++)
                            ...
                            {
                                data.image[ID_out]
                                    .array.CF[kk * naxes_out[0] * naxes_out[1] + jj * naxes_out[0] + ii]
                                    .re += data.image[ID]
                                               .array
                                               .CF[(kk * n3 + k) * naxes[0] * naxes[1] + (jj * n2 + j) * naxes[0] +
                                                   ii * n1 + i]
                                               .re;
                                data.image[ID_out]
                                    .array.CF[kk * naxes_out[0] * naxes_out[1] + jj * naxes_out[0] + ii]
                ...
                for (uint32_t kk = 0; kk < naxes_out[2]; kk++)
                    for (int i = 0; i < n1; i++)
                        for (int j = 0; j < n2; j++)
                            ...
                            {
                                data.image[ID_out]
                                    .array.CD[kk * naxes_out[0] * naxes_out[1] + jj * naxes_out[0] + ii]
                                    .re += data.image[ID]
                                               .array
                                               .CD[(kk * n3 + k) * naxes[0] * naxes[1] + (jj * n2 + j) * naxes[0] +
                                                   ii * n1 + i]
                                               .re;
                                data.image[ID_out]
                                    .array.CD[kk * naxes_out[0] * naxes_out[1] + jj * naxes_out[0] + ii]
