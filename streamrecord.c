...
imageID IMAGE_BASIC_streamrecord(const char *__restrict streamname, long NBframes, const char *__restrict IDname);
...
errno_t __attribute__((cold)) streamrecord_addCLIcmd()
{
    ...
    RegisterCLIcommand("imgstreamrec", __FILE__, IMAGE_BASIC_streamrecord_cli, "record stream of images",
                       "<stream> <# frames> <output>", "imgstreamrec imstream 100 imrec",
