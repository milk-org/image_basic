...
imageID basic_rotate(const char *__restrict ID_name, const char *__restrict IDout_name, float angle)
{
    ...
    for (uint32_t jj = 0; jj < naxes[1]; jj++)
        ...
        {
            long iis = (long)(naxes[0] / 2 + (ii - naxes[0] / 2) * cos(angle) + (jj - naxes[1] / 2) * sin(angle));
            long jjs = (long)(naxes[1] / 2 - (ii - naxes[0] / 2) * sin(angle) + (jj - naxes[1] / 2) * cos(angle));
...
imageID basic_rotate90(const char *__restrict ID_name, const char *__restrict ID_out_name)
{
    ...
    for (uint32_t jj = 0; jj < naxes[0]; jj++)
        for (uint32_t ii = 0; ii < naxes[1]; ii++)
        {
            ...
            data.image[IDout].array.F[jj * naxes[1] + ii] = data.image[ID].array.F[jjs * naxes[0] + iis];
        }

    return IDout;
}

imageID basic_rotate_int(const char *__restrict ID_name, const char *__restrict ID_out_name, long nbstep)
{
    float angle;
    imageID ID;
    imageID IDout;
...
imageID basic_rotate2(const char *__restrict ID_name_in, const char *__restrict ID_name_out, float angle)
{
    imageID ID_in;
