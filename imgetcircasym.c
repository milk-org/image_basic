...
imageID IMAGE_BASIC_get_circasym_component(const char *__restrict ID_name, const char *__restrict ID_out_name,
                                           float xcenter, float ycenter, const char *options);
...
static errno_t IMAGE_BASIC_get_circasym_component_cli()
{
    ...
    {
        IMAGE_BASIC_get_circasym_component(data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.string,
...
errno_t __attribute__((cold)) imgetcircasym_addCLIcmd()
{
    ...
    RegisterCLIcommand("imgetcircasym", __FILE__, IMAGE_BASIC_get_circasym_component_cli,
                       "extract non-circular symmetric part of image", "<inim> <outim> <xcenter> <ycenter>",
                       "imcgetcircassym imin imout 256.0 230.5",
                       "long IMAGE_BASIC_get_asym_component(const char *ID_name, const char *ID_out_name, float "
                       "xcenter, float ycenter, const char *options)");

    return RETURN_SUCCESS;
}

imageID IMAGE_BASIC_get_circasym_component_byID(imageID ID, const char *__restrict ID_out_name, float xcenter,
        ...
        for (uint32_t ii = 0; ii < naxes[0]; ii++)
        {
            ...
            if ((i + 1) < nb_step)
            {
                data.image[IDout].array.F[jj * naxes[0] + ii] =
                    data.image[ID].array.F[jj * naxes[0] + ii] - ((1.0 - x) * mean[i] + x * mean[i + 1]);
            }
        }
